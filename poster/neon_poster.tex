...
\hrule\medskip
\Head{Conclusion}\\
A new norm-motivated algorithm proved to be not so effective as Muon, at least from the first sight. However, its updates seem to be much faster to compute, and more so for a large matrices which has not tested yet. We believe that our algorithm is not an alternative to Muon, but probably a more effective tool for solving some other optimization problem. A deep inspection of Nuclear Norm and its properties might give us an insight into Neon's future.
