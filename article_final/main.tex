...
For gradient norm profiling and results for training with Fanions and F-Fanions, please see \sectionref{sec:cifar_underhood}.
...
A common limitation of these analyses is their focus on convergence measured by the gradient norm. As we showed in our CIFAR experiments \sectionref{sec:cifar_underhood}, the gradient norm may decrease by only a factor of ten when the accuracy reaches 100\%.
...
Unfortunately, $\normfstar{\cdot}^\dagger$ and $\normftwo{\cdot}^\dagger$ do not have closed-form expressions and cannot be found as easy as their duals.
...
Tuned \texttt{lr} and \texttt{momentum} coefficients are in the \tableref{tbl:lls_lrs_app}. Spectral, nuclear norm of the gradients over iterations and loss over time are in the \figureref{fig:app_lls}. Very low speed of Fanions and F-Fanions for large $k$ is probably caused by ineffective implementation of TRLan by \cite{cupy_svds_ref}, since $\mathcal{O}(mn^2 + n^2k + nk^2)$ complexity is actually $\mathcal{O}(mn^2)$.

\begin{table}[htbp]
\floatconts
  {tbl:lls_lrs_app}
  {\caption{Tuning \texttt{lr} and \texttt{momentum} coefficients}}
  {%
    \begin{tabular}{|c|c|c|c|}
      \arrayrulecolor{black}\hline
      Algorithm & \texttt{lr}  & \texttt{momentum}  & Iterations to 0.001 loss \\
...
\floatconts
  {fig:app_lls}
  {\caption{More plots for linear least squares problem for a 500x500 matrix}}
  {%
    \subfigure[The spectral norm of the gradient]{\label{fig:lls_op_grad_norm}%
      \includegraphics[width=0.5\linewidth]{figs/simple_lls/gradient_spectral_norm_vs_iteration_long_run.pdf}}%
    \vspace{1em}
    \subfigure[The nuclear norm of the gradient]{\label{fig:lls_nuclear_grad_norm}%
      \includegraphics[width=0.5\linewidth]{figs/simple_lls/gradient_nuclear_norm_vs_iteration_long_run.pdf}}%
    \vspace{1em}
    \subfigure[The loss over time]{\label{fig:lls_loss_time}%
      \includegraphics[width=0.5\linewidth]{figs/simple_lls/loss_vs_time_long_run.pdf}}
  }
\end{figure}
...
Most bounds for Muon and other LMO-algorithms are given for the norm of the gradient \citep{li2025noteconvergencemuon, kovalev2025understanding, pethick2025training, riabinin2025gluon,kovalev2025noneuclideansgdstructuredoptimization}. Accordingly, it is natural to measure those norms on a real deep learning problem.
...
We compare Muon, F-Muon, S-Muon, NSGD, SignSGD, Neon, F-Neon, Fanion-5, and F-Fanion-5 on CIFAR-airbench. NSGD and SignSGD are not heavily tuned. The hyperparameters of Neon, F-Neon, Fanion-5, and F-Fanion-5 are taken from Muon and F-Muon (see \tableref{tbl:cifar_lrs}). In the table, val\_accuracy after 8 epochs is for the the airbench variant with weight normalization at each step. However, in order to be faithful to the conditions of the convergence theorems \citep{kovalev2025understanding,riabinin2025gluon}, we do not normalize the weights of the network when logging gradient norms.

We log train and validation accuracies (\figureref{fig:cifar_accs}), and nuclear, gradient, and spectral norms of the gradients of conv1.weight and conv2.weight for all the layers (see \figureref{fig:norms_conv1_vs_conv2} for norms in the layer 2, and \figureref{fig:total_norms_cifar} for total norms of the weights). During training, the gradient norms decrease only by the order of ten maximum, while train accuracy reaches full 100\% for Muon, S-Muon, and F-Muon. This means that the bounds of the $\max\{\frac{A}{\epsilon}, \frac{B}{\epsilon^2}, \frac{C}{\epsilon^3}, \frac{D}{\epsilon^4}\}$ type (like Corollary 2 from \citet{kovalev2025understanding}) probably should not be reduced to $\frac{D}{\epsilon^4}$ when selecting the optimal algorithm for a practical problem.
...
\paragraph{Linear Least Squares and CIFAR airbench} Experiments were conducted on a single NVIDIA RTX A4000. The code was run with Python 3.10.13 and Pytorch~2.6.0+cu126.
