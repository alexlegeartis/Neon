...
Deriving the update for arbitrary $k$: Since $\normkfk{\cdot}^\dagger = \max\{\frac{1}{k} \normn{\cdot}, \norms{\cdot}\}$ (see \citep{bhatia2013matrix}, p. 96), the goal is to reach $\max\{\frac{1}{k}\sum_{i=1}^{\min(m,n)}\sigma_i, \sigma_1\}$. The Neon update $\Delta = \vu_1 \vv_1^\top$ achieves $\sigma_1$, while the scaled Muon update $\Delta = \frac{1}{k}\sum_{i=1}^{\min(m,n)}\vu_i \vv_i^\top$ achieves $\frac{1}{k}\sum_{i=1}^{\min(m,n)}\sigma_i$. Thus, the update is either
\[
...
First, we need a well-known fact mentioned, for example, in \citep[Table 1]{yu2012arithmetic}. For the sake of completeness, we provide a proof here.
...
\end{equation}
where $\mX \in \Rmn$, $m=500$ and $n=500$ are typical dimensions of a neural network weight matrix, $\mS \in \Rmn$, $\mM \in \mathbb{S}^m_{+}$ and $\mN \in \mathbb{S}^n_{+}$ are positive-semidefinite matrices. The spectra of $\mM$ and $\mN$ are uniformly distributed in the $(0, 1)$ interval. We set $\mS=0$, and elements of $\mX^0$ are instances of random value with distribution $\sim 0.1 \cN(0, 1)$.
