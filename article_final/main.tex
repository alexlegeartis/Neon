...
In this case,
\[
\argmin_{\mD \in \cS} \<\mM^t, \mD> = - \{ \mD \in \cB_1 \mid \<\mM^t, \mD> = \norm{\mM^t}^\dagger\}\,,
\]
...
\[
\mX^{t+1} = \mX^t - \eta_t \mU \frac{\diag\left(\sigma_1^{q-1},\dots, \sigma_{\min(m,n)}^{q-1}\right)}{\left(\sum_{i=1}^{\min(m,n)}{\sigma_i^q}\right)^{\frac{q-1}{q}}}\mV^\top
\]
