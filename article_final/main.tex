...
The per-iteration complexity of TRLan is $\mathcal{O}(mn)$, where $m \geq n$ are the dimensions of the target matrix.
...
The spectral and nuclear norms of the gradients over iterations, as well as the loss over time, are shown in \figureref{fig:app_lls}. The poor performance of Fanions and F-Fanions in terms of speed for large $k$ is likely caused by the inefficient implementation of TRLan.
