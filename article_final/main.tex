...
Our experiments reveal important insights into the role of matrix norms in optimization. First, using the example of Neon (the rank-one Fanion), we show that not every LMO-based algorithm is effective, despite sharing the same asymptotics in the general bounds of \citet{kovalev2025understanding} and \citet{riabinin2025gluon}. This suggests that existing theoretical guarantees should be refined to better explain empirical performance.
...
Here, $\mM^t = \mU \mSigma \mV^\top$ is the Singular Value Decomposition (SVD) of $\mM^t$ ($\mU = [\vu_1, \vu_2, \dots, \vu_r]$, $\mSigma = \diag(\sigma_1, \sigma_2, \dots, \sigma_r)$, and $\mV = [\vv_1, \vv_2, \dots, \vv_r]$). Muon can be recovered by taking the RMS-to-RMS operator norm $\sqrt{\frac{n}{m}}\norms{\cdot}$.
...
Having examined the Frobenius norm (NSGD), spectral norm (Muon), and Chebyshev norm (SignSGD), all of which produce full-rank updates, we naturally turn to the nuclear norm $\normn{\cdot}$.
...
\end{lemma}
\begin{proof}
Since $\normn{\cdot}^\dagger = \norms{\cdot}$, the goal is to reach $\norms{\mM^t} = \sigma_1$. Note that $\Delta = \vu_1 \vv_1^\top$ delivers this value. Indeed, $\normn{\Delta}=1$ and by the trace property and orthogonality of the singular vectors,
\[
\<\mM^t, \Delta> =\<\mU \mSigma \mV^\top, \vu_1 \vv_1^\top> = \tr \diag({\sigma_1, 0, \dots, 0}) = \norms{\mM^t}\,,
\]
...
\]
where $p$ and $q$ satisfy $\frac{1}{p} + \frac{1}{q} = 1$. This formula recovers Neon when $p\rightarrow1$ (provided $\sigma_1 > \sigma_2$, which holds on real data), NSGD when $p=2$, and Muon when $p\rightarrow\infty$.

However, Schatten norms do not fill the rank gap: when $p > 1$, the update has full rank, while when $p=1$ (or $p \rightarrow 1$), it has rank one. Moreover, computing the update for $p \neq 1, 2, \infty$ appears to require knowing all $\sigma_i$, making the problem as hard as computing the full SVD.
...
Deriving the update for arbitrary $k$: Since $\normkfk{\cdot}^\dagger = \max\{\frac{1}{k} \normn{\cdot}, \norms{\cdot}\}$ (see \citet{bhatia2013matrix}, p. 96), the goal is to reach $\max\{\frac{1}{k}\sum_{i=1}^{\min(m,n)}\sigma_i, \sigma_1\}$. The Neon update $\Delta = \vu_1 \vv_1^\top$ achieves $\sigma_1$, while the scaled Muon update $\Delta = \frac{1}{k}\sum_{i=1}^{\min(m,n)}\vu_i \vv_i^\top$ achieves $\frac{1}{k}\sum_{i=1}^{\min(m,n)}\sigma_i$. Thus, the update is either
\[
\mX^{t+1} = \mX^{t} - \gamma_t \vu_1 \vv_1^\top\text{ or }\mX^{t+1} = \mX^{t} - \frac{\gamma_t}{k}\mU \mV^\top\,,
\]
depending on which better minimizes the linear function $L(\mX) \equiv F(\mX^t) + \mM^t(\mX - \mX^t)$.

The Ky Fan norms thus fail to close the rank gap: the resulting update is either rank-one or full-rank.

\subsection{Solution: Duals to Ky Fan Norms}

While Schatten norms are closed under dualization and $\norm{\mM^t}_{S_p}^\dagger = \norm{\mM^t}_{S_q}$, for Ky Fan norms it is not generally the case. $k = 1$ and $k=\min(m, n)$ are exceptional: for $k=1$ the dual norm is $\normn{\cdot}$ with $k = \min(m, n)$ and for $k = \min(m, n)$ the dual norm is $\norms{\cdot}$ with $k=1$. For each other $k$, $\normkfk{\cdot} \neq \norm{\cdot}_{\text{KF-}k'}$ for any $k'$: $k'=\min(m, n)$ and $k'= 1$ correspond to the already discussed cases, while for other $k'$ one can change $\norm{\mM^t}_{\text{KF-}k'}$ by moving a small value between $\sigma_{k'}$ and $\sigma_{k'+1}$. During this change, $\norms{\mM^t}$ and $\normn{\mM^t}$ will remain constant, hence $\normkfk{\mM^t}$ will not change as well.

The key insight is to work with the duals of Ky Fan norms:
...
These updates constitute the family of \emph{Fanions}, LMO-based algorithms under the $\normkfk{\mM^t}^\dagger$ norms. We call the algorithm for a particular $k$ \emph{Fanion-$k$}. This family elegantly bridges the rank gap with updates of any intermediate rank $k$.

\paragraph{Connection to existing algorithms}
The Fanion family unifies several known algorithms as special cases:
\begin{itemize}
	\item \textbf{Neon} is Fanion-1 (rank-one updates)
	\item \textbf{Muon} is Fanion-$\min\{m,n\}$ (full-rank updates)
	\item \textbf{Dion} (unsharded): The rank-$r$ Dion (Algorithm 1 from \citet{ahn2025dion}) without error feedback and without scaling of the update is actually Fanion-$r$ (see \citet{pethick2025understandingdion}, where Dion is written in a notation more similar to ours)
\end{itemize}
...
Simply applying the norm dual to the Ky Fan $k$-norm, however, is not enough to provide a family of algorithms diverse enough for our purposes. Next, we consider linear combinations of algorithms from the LMO framework, which happen to be LMO-algorithms as well, as we show in the following paragraphs.
...
The norm $\norm{\cdot}_{\mathrm{C-KF-k}}^\dagger$ that produces this update in the LMO framework is again given by \corollaryref{corollary:lin_comb_alg}:
...
One cannot but notice the similarity between the updates of LMO optimizers in the vector $l_p$ and the matrix Schatten $S_p$ norms, which is illustrated by \tableref{tbl:mat_vs_vec_lmo}. The parallels are not limited to mere similarity of the updates and can be traced even in the empirical observations. SignSGD is very close to Adam, as noted in \citet{bernstein2024oldoptimizernewnorm}, and both Adam and Muon perform well in training large models \citep{zhao2025deconstructing, liu2025muon}. NSGD is the same for both matrix and vector cases. Greedy Coordinate Descent is not applied to high-dimensional problems, so from this perspective it is not surprising that one-rank Neon underperforms on such problems.
...
\paragraph{NanoGPT pretrain} We used the standard setting of 8 $\times$ H100 as documented in \citet{modded_nanogpt_2024}. 
