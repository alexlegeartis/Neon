...
\end{equation}
where $\mM^t$ is the gradient or momentum buffer and $\cS \subset \Rmn$ is some constraint set. The algorithm proceeds as follows: first compute the momentum buffer $\mM^t$, then update $\mX^{t+1}$:
\begin{equation}\label{eq:simple_update}
	\begin{aligned}
	\mM^{t} &= \begin{cases}
		\nabla F(\mX^t) & \text{(no momentum)},\\
		\beta \mM^{t-1} + (1 - \beta) \nabla F(\mX^t) & \text{(heavy ball)},\\
    \beta \mM^{t-1} + (1 - \beta) \nabla F\left(\mX^{t - 1} + \gamma_t\mathrm{LMO}\left(\mM^{t-1}\right)\right) & \text{(Nesterov)},
	\end{cases}\\
	\mX^{t + 1} &= \mX^t + \gamma_t\mathrm{LMO}\left(\mM^t\right)\,.\\
	\end{aligned}
\end{equation}
Here $\beta \in [0, 1)$ is the momentum coefficient. Throughout our experiments, we employ Nesterov momentum unless otherwise specified.
...
We evaluate several algorithms from the Fanion family and their convex combinations, using the update rule from \equationref{eq:simple_update} with Nesterov momentum:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \texttt{lr} $\in [0.00005, 0.00020]$ with step $0.00001$ for SignSGD.
...
We test F-Muon and S-Muon with $\alpha=0.5$ on the NanoGPT speedrun \citep{modded_nanogpt_2024}. The optimal parameters are in the legend \figureref{fig:gpt_loss}, \texttt{sign\_sgd\_coeff=0.0003/0.07}. After testing for 1750 steps we get 3.281 cross-entropy loss for F-Muon and 3.287 for S-Muon. For Muon, it falls below the target threshold 3.28 and reaches 3.279. However, this difference is negligible if one looks at \figureref{fig:gpt_loss}. It is even more striking, considering the fact that this F-Muon is an average of Muon and NSGD, and the latter performed quite poorly. The same is true for S-Muon.
...
Again, as on the CIFAR airbench, if for F-Muon we set $\alpha=-0.1$ that corresponds to no ball, we get a 3.2818 loss, which is a small difference from the baseline.

It is noteworthy that F-Muon with NSGD-part that is not scaled by Muon's rule gives 3.288 loss (0.007 increase), while S-Muon scaled with Muon's rule gives 3.292 loss (0.005 increase)

\subsection{GPT-2 Medium Speedrun}
We scale from NanoGPT to GPT-2 Medium. After testing for 5960 steps with F-Muon we get 2.9215 loss, while for Muon it is 2.9198. In ? steps, F-Muon reaches <2.92 loss, thus needing only ?\% more iterations than Muon. 
