...
\end{equation}
where $\mM^k$ is the gradient (or momentum) of $F$ at $\mX^k$ and $\cS \subset \Rmn$ is some set. The update of the algorithm is defined as follows:
...
\]
and the update for $\mX^k$ in \equationref{eq:simple_update} simplifies to
...
\begin{lemma}\label{lemma:neon_update}
	...
	\begin{equation}\label{eq:neon_update}
		\mX^{k+1} = \mX^{k} - \gamma_k \vu_1 \vv_1^\top\,.
	\end{equation}
\end{lemma}
\begin{proof}
