%\documentclass[wcp,gray]{jmlr} % test grayscale version
 ...
 \begin{abstract}
   In this article, we explore the use of various matrix norms for optimizing functions of weight matrices, a crucial problem in deep learning. Moving beyond the spectral norm that underlies the Muon update, we leverage the duals of the Ky Fan $k$-norms to introduce to introduce the \emph{Fanion} family of linear minimization oracle (LMO) algorithms, which are closely related to Muon, $\nu$-SAM, and Dion. Staying inside the LMO, we construct the families of \emph{F-Fanions} and \emph{S-Fanions}, whose updates are a convex combination of the updates of Fanions and Normalized SGD or SignSGD, respectively. The most promising algorithms in these familes are \emph{F-Muon} and \emph{S-Muon}. By conducting an extensive empirical study of all three algorithm families across a wide range of tasks and settings, we demonstrate that F-Muon and S-Muon consistently match Muon's performance, while outperforming Muon on a synthetic smooth convex problem.
 \end{abstract}
 ...
 \begin{itemize}
   \item Synthetic smooth convex problem for a matrix argument
 ...
 \floatconts
   {tbl:lls_lrs}
   {\caption{Tuned learning rates and momentum coefficients in the experiments on the smooth convex problem.}}
   {%
     \begin{tabular}{cccc}
       \arrayrulecolor{black}\toprule
 ...
 \floatconts
   {fig:lls}
   {\caption{A smooth convex problem for a 500x500 matrix.}}
   {%
 ...
 Another way to understand Muon is to focus on particular problems, like multi-class linear classification on separable data \citep{fan2025implicit}. Curiously, Neon has the same margin convergence rate as Muon because both are based on Schatten norms. That being said, the norms for margins are different: Muon has implicit bias toward solutions that minimize spectral norm of the weight matrix, while Neon tends to minimize its nuclear norm.
 ...
 \floatconts
   {fig:app_lls}
   {\caption{More plots for the smooth convex problem for a 500x500 matrix.}}
   {%
 ...
 \paragraph{The smooth convex problem and CIFAR airbench} Experiments were conducted on a single NVIDIA RTX A4000. The code was run with Python 3.10.13 and Pytorch~2.6.0+cu126 on a computer with an EPYC 7543 processor.
