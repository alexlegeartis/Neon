...
Tuned \texttt{lr} and \texttt{momentum} coefficients are presented in \tableref{tbl:lls_lrs_app}. The spectral and nuclear norms of the gradients over iterations, as well as the loss over time, are shown in \figureref{fig:app_lls}. The poor performance of Fanions and F-Fanions in terms of speed for large $k$ is likely caused by the inefficient implementation of TRLan.

\begin{table}[htbp]
\floatconts
  {tbl:lls_lrs_app}
  {\caption{Tuned \texttt{lr} and \texttt{momentum} coefficients in the lease squares experiment.}}
  {%
    \begin{tabular}{|c|c|c|c|}
      \arrayrulecolor{black}\hline
      Algorithm & \texttt{lr}  & \texttt{momentum}  & Iterations to 0.001 loss \\
