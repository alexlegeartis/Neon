...
\begin{block}{Problem}
$\argmin_{\mX\in\R^{m\times n}} f(\mX)$; especially when $\mX$ is a weight matrix in a neural network 
\end{block}

\begin{block}{Goals}
Utilize the matrix structure beyond the spectral norm and Muon to get other fast algorithms
\end{block}

\begin{block}{Solution}
Generalization of Muon to Fanions that shed light on Dion; combinations of duals to Ky Fan norms with Frobenius or Chebyshev norms to induce algorithms that combine Fanions with NSGD or SignSGD
\end{block}
\end{frame}
%-------------------------------------------------------------------------------------
...
$\mathrm{LMO}(\mM^t) \in \argmin_{\mD \in \cS} \langle\mM^t, \mD\rangle\,,$ where $\mM^t$ is either gradient or momentum, leads to the update
$$\mX^{t+1} = \mX^{t} - \eta \arg\max_{\mX \in \cB_1} \<\mM^t, \mX> = \mX^{t} - \eta \{\Delta \in \cB_1 \mid \<\mM^t, \Delta> = \norm{\mM^t}^\dagger\}$$

\vspace{0.4em}

\textbf{Recipe}: We seek $\Delta$ from the 1-norm ball that delivers $\<\mM^t, \Delta> = \norm{\mM^t}^\dagger$
\vspace{0.4em}

We will utilize the SVD of $\mM^t$: $\mM^t = \mU \Sigma \mV^\top$.
\end{frame}
...
\end{frame}
%-----------------------------------------------------------------------------------------------------------
\begin{frame}{Geometric point of view: Ky Fan norms are edged}
  \centering
...
\end{frame}
%-----------------------------------------------------------------------------------------------------------
\begin{frame}{$\normfkfk{\mM^k}^\dagger$ and F-Fanions, $\norm{\cdot}^\dagger_{\mathrm{C^\dagger-KF-k}}$ and S-Fanions}

  $\alpha \times$Fanion ($\normkfk{\cdot}^\dagger$ norm) + $(1-\alpha) \times$NSGD ($\normf{\cdot}$ norm) = F-Fanion:
  $$\mX^{t+1} = \mX^{t} - \eta \left(\alpha \sum_{i=1}^k{u_i v_i^\top} + (1-\alpha)\frac{\mM^t}{\normf{\mM^t}}\right)$$

  $\alpha \times$Fanion + $(1-\alpha)\eta\times$SignSGD ($||\cdot||_\infty \equiv ||\cdot||_C$ norm) = S-Fanion:  
...
\begin{frame}{A smooth convex problem for a matrix}
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \centering
      \scriptsize Frobenius norm of the gradient
    \end{column}
  \end{columns}
  \vspace{0.3em}
  \centering
