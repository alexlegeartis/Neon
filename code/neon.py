...
torch.backends.cudnn.enabled = False    # Disable CUDA-specific optimizations
...
def main(run, model):
    ...
    # Use CPU for data loading and move to GPU only when needed
    test_loader = CifarLoader('cifar10', train=False, batch_size=batch_size)
    train_loader = CifarLoader('cifar10', train=True, batch_size=batch_size, aug=dict(flip=True, translate=2))
    if run == 'warmup':
        train_loader.labels = torch.randint(0, 10, size=(len(train_loader.labels),), device='cpu')
    ...
    def stop_timer():
        nonlocal time_seconds
        time_seconds = time.time() - time_seconds
