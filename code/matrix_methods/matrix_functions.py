...
def several_sv_svds_approximation(W_torch, k, num_iter=50):
    ...
    # Check dimensions before converting to CuPy
    m, n = W_torch.shape[0], W_torch.shape[1]
    min_dim = min(m, n)
    ...
    if k_val <= 0 or k_val >= min_dim:
        ...
        if W_norm < 1e-12:
            m, n = W_torch.shape
            U = torch.zeros(m, 1, device=original_device, dtype=original_dtype)
            S = torch.zeros(1, device=original_device, dtype=original_dtype)
            Vt = torch.zeros(1, n, device=original_device, dtype=original_dtype)
            return U, S, Vt
        m, n = W_torch.shape
        v_normalized = W_torch / (W_norm + 1e-12)
        if m == 1:
            U = torch.ones(1, 1, device=original_device, dtype=original_dtype)
            S = torch.tensor([W_norm], device=original_device, dtype=original_dtype)
            Vt = v_normalized.reshape(1, n)
        else:
            U = v_normalized.reshape(m, 1)
            S = torch.tensor([W_norm], device=original_device, dtype=original_dtype)
            Vt = torch.ones(1, 1, device=original_device, dtype=original_dtype)
        return U, S, Vt
    
    U, S, Vt = cupyx_svds(W, k=k_val, maxiter=num_iter, which='LM')
...
def one_sv_svds_approximation(W_torch, num_iter=30):
    ...
    if min_dim < 4:
        # Use full SVD for small matrices
        U, S, Vt = torch.linalg.svd(W_torch, full_matrices=False)
        # Return rank-1 approximation: U @ Vt (without singular value scaling)
        approx_torch = U[:, 0:1] @ Vt[0:1, :]
        return approx_torch, float(S[0])
    
    W = cp.from_dlpack(thd.to_dlpack(W_torch)).astype(cp.float32)
    # Ensure k is valid: k must be >= 1 and < min(m, n)
    k_val = min([k, min_dim - 1])
    if k_val <= 0 or k_val >= min_dim:
        # Fallback for invalid k values (shouldn't happen given above check, but safety)
        W_norm = W_torch.norm().item()
        if W_norm < 1e-12:
            approx_torch = torch.zeros_like(W_torch)
            return approx_torch, 0.0
        approx_torch = W_torch.clone()
        return approx_torch, float(W_norm)
