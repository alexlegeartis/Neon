# BatchNorm does not work for us here, because we want to find gradients of losses on each data.
"""
...
def main(run, model):
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # single-sample loss (we keep same label_smoothing as your main loss)
            def single_loss(params, buffers, x, y):
                # fmodel expects batched input, so unsqueeze to make a batch of size 1
                out = fmodel(params, buffers, x.unsqueeze(0))
                # return scalar per-sample loss
                return F.cross_entropy(out, y.unsqueeze(0), label_smoothing=0.2, reduction="sum")

            # grad function that returns a tuple/list of grads (one tensor per param)
            grad_fn = grad(single_loss)

            # function that returns the L2 norm of the per-parameter grads for a single sample
            def per_sample_grad_norm(params, buffers, x, y):
                grads = grad_fn(params, buffers, x, y)  # grads is a pytree matching params
                # replace any None grads with zeros (safety)
                flat_list = []
                for g, p in zip(grads, params):
                    if g is None:
                        flat_list.append(torch.zeros_like(p).reshape(-1))
                    else:
                        flat_list.append(g.reshape(-1))
                flat = torch.cat(flat_list, dim=0)
                # L2 norm
                return flat.norm(p=2).clamp_min(1e-12)

            # Vectorize over the batch dimension (params and buffers are not batched -> in_dims None)
            # in_dims = (None, None, 0, 0) means params->None (same for all vmapped calls),
            # buffers->None, x batched at dim0, y batched at dim0
            per_sample_norms = vmap(per_sample_grad_norm, in_dims=(None, None, 0, 0))(
                params, buffers, inputs.to(next(model.parameters()).device), labels.to(next(model.parameters()).device)
            )
            # per_sample_norms : tensor shape [B], on same device as params

            # OPTIONAL: move to CPU for logging / storing if you don't need them on GPU
            per_sample_norms_cpu = per_sample_norms.detach().cpu()
