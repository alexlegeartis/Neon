...
def main(run, model):
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            per_sample_losses = F.cross_entropy(outputs, labels, label_smoothing=0.2, reduction="none")
            batch_size = len(inputs)
            
            # Compute gradients for each sample using vmap
            # Extract model parameters for functional API
            # Ensure BatchNorm parameters are float32 (they should be, but double-check)
            params_dict = {}
            for n, p in model.named_parameters():
                # BatchNorm parameters must be float32
                if any(norm_name in n for norm_name in ['norm1', 'norm2']):
                    params_dict[n] = p.float() if p.dtype == torch.float16 else p
                else:
                    params_dict[n] = p
            param_names = list(params_dict.keys())
            params_tuple = tuple(params_dict.values())
            
            # Temporarily disable BatchNorm state updates to avoid in-place operations
            bn_modules = [m for m in model.modules() if isinstance(m, BatchNorm)]
            bn_states = []
            for bn in bn_modules:
                ...
                bn_states.append({
                    ...
                    'num_batches_tracked': bn.num_batches_tracked
                })
                # Disable tracking and set to eval to prevent all state updates
                bn.track_running_stats = False
                bn.eval()
            
            def compute_loss_single(params_tuple, input_single, label_single):
                """Compute loss for a single sample given model parameters"""
                # Reconstruct params dict from tuple (dtypes should already be correct)
                params_dict_local = {name: p for name, p in zip(param_names, params_tuple)}
                
                # Ensure input is in the right dtype (half precision for most layers)
                input_for_model = input_single.unsqueeze(0)
                
                output_single = functional_call(
                    model, 
                    params_dict_local, 
                    (input_for_model,),
                    {"whiten_bias_grad": (step < whiten_bias_train_steps)}
                )
                return F.cross_entropy(output_single, label_single.unsqueeze(0), label_smoothing=0.2, reduction="mean")
            
            def compute_grad_norm_single(input_single, label_single):
                """Compute gradient norm for a single sample"""
                # Compute gradients w.r.t. parameters (argnums=0 means differentiate w.r.t. first argument)
                grads_tuple = grad(compute_loss_single, argnums=0)(params_tuple, input_single, label_single)
                total_norm = 0.0
            ...
            per_sample_grad_norms = vmap(compute_grad_norm_single)(inputs, labels)
            
            # Restore BatchNorm modules to original state
            for bn, state in zip(bn_modules, bn_states):
                bn.track_running_stats = state['track_running_stats']
                if state['training']:
                    bn.train()
                else:
                    bn.eval()
                # Note: num_batches_tracked may have been updated, but that's okay for training
            
            # Print gradient norms
            for i in range(batch_size):
                print(f"Step {step}, Sample {i}: gradient_norm = {per_sample_grad_norms[i].item():.6f}")
            
            # Now compute aggregated gradients for optimizer step
