...
\begin{equation}\label{eq:simple_update}
	\mX^{k + 1} = \mX^k + \gamma_k\mathrm{LMO}\left(\mM\right)\,.
\end{equation}

We are particularly interested in the case when $\cS$ is a ball in the norm $\norm{\cdot}$:

$$\cS = \cB_\eta \in \{ \mD \in \Rmn \mid \norm{\mD} \leq \eta \}.$$
In this case,

$$\argmin_{\mD \in \cS} \<\mM^k, \mD> = - \eta \{ \mD \in \cB_1 \mid \<\mM^k, \mD> = \norm{\mM^k}^\dagger\}$$

and update for $\mX^k$ in \cref{eq:simple_update} simplifies to
\begin{equation}\label{eq:our_update}
	\mX^{k+1} = \mX^{k} - \eta_k \{\mD \in \cB_1 \mid \<\mM^k, \mD> = \norm{\mM^k}^\dagger\}\,
\end{equation}

By this formula, it is easy to compute updates for algorithms induced by various norms $\norm{\cdot}$. For example, when the norm $\norm{\cdot}$ is the Frobenius norm $\normf{\cdot}$, \cref{eq:our_update} turns into

\begin{equation}\label{eq:nsgd_update}
	\mX^{k+1} = \mX^{k} - \eta_k \frac{\mM^k}{\normf{\mM^k}},
\end{equation} which recovers Normalized SGD (NSGD). When one has the spectral norm $\norms{\cdot}$, one gets

\begin{equation}\label{eq:muon_update}
	\mX^{k+1} = \mX^{k} - \eta_k U V^\top,
\end{equation} which is Muon without the $\sqrt{m/n}$ factor.
...
\citet{cesista2025schattenp} considered Schatten-$p$ norms $\norm{\mM^k}_{S_p} = \left(\sum_{i=1}^{\min(m, n)}\sigma_i^p\right)^{1/p}$. The update was as follows: $$X^{k+1} = X^k - \eta_k \mU \frac{\diag\left(\sigma_1^{q-1},\dots, \sigma_{\min(m,n)}^{q-1}\right)}{\norm{\mM^k}^{q-1}_{S_q}}\mV^\top$$ for $q$: $\frac{1}{p} + \frac{1}{q} = 1$. This formula recovers Neon when $p\rightarrow1$ provided that $\sigma_1 > \sigma_2$, which is true on real data; NSGD when $p=2$, and Muon when $p\rightarrow\infty$.
...
\begin{lemma}\label{lemma:ky_fan_update}
	...
	\end{equation}
\end{lemma}
In the section ~\cref{sec:matrix-side}, we will discuss how to compute the updates efficiently.
...
Add more about Dion, and what its difference from us. I don't understand why their algorithm works well. I don't believe that the problem is with the error feedback.

\section{Algorithms for Matrices vs Algorithms for Vectors}
...
NSGD, Muon, and SignSGD work fair enough separately. Could we boost the quality if we combined them? To answer this question, we have to build a theoretical base for that.

\subsection{Duals to Convex Combination of Norm}
...
Now we can work with different norms.

\subsection{General case}
We have norm \#1 with update \#1 and norm \#2 with update \#2. We prove the lemma that says that the update of their conic combination is a conic combination of updates \#1 and \#2.

Then prove a lemma for any set of norms \#1, \dots, \#n (by induction)

\subsection{Frobeniusize them: F-Muon and F-Neon}
Let us consider a mixture of Fanions with another excellent LMO-algorithm, NSGD.
...
We name the derived algorithm \emph{F-Muon}.
...
First, we compare F-Fanions on the following L-smooth problem:
...
We suppose that the reason for the recorded by us discrepancy between Neon and Muon performance, both of which are described by Scion or Gluon frameworks, lies in the structure of the norm ball, which must be an object of further research.
	...
	{\bf Orthogonal research} Fanions, F-Fanions, and S-Fanions benefit from the general theoretical description of \citet{riabinin2025gluon}. They could be transformed to Drop-Fanions etc. (cite Drop-Muon article), they could be viewed as Broximal Point Methods (cite the article), they could be subject to EF (cite the EF for all). Of course, they could be clipped (cite ClipScion), and they could be analyzed when the LMO is not calculated accurately (cite Beyond the Ideal).
