...
That is what can be said from a practical point of view. \replaced[id=IK, comment={Rewrote this to be more precise. The innovation wasn't just 'using' a norm, but deriving the update from a principled, constrained optimization problem. I think this better reflects the contribution of the Muon paper.}]{From a theoretical perspective, a key innovation of Muon was its principled derivation of the update rule, which emerges as the solution to an optimization problem constrained by the spectral norm \citep{bernstein2025deriving}}{From the perspective of theory, the main innovation of Muon has been an intentional usage of matrix norms, i.e. the spectral norm, to derive the algorithm's update}. 

Motivated by the success of Muon, many generalizations and variations of it were proposed. Among the notable ones are Scion \citep{}, Deon \citep{} and Gluon \citep{}. One question still persists however: Why 

Based upon recent theoretical advances that explain some theory behind Muon, Scion and Gluon \citep{bernstein2025deriving,kovalev2025understanding,pethick2025training,riabinin2025gluon}, we explore application of other matrix norms to optimization of functions of matrices. As it has been done with Muon, we stipulate that our algorithms' updates be fast to compute.

\replaced[id = IK, comment={Subsequent paragraphs in the introduction are almost completely new.}]{}{}

%In this article, \replaced[id=IK, comment={I think this is much clearer to introduce all algorithms like that, the paragraphs where one of every two words is a ***on are confusing.}]{we generalize the spectral-norm-based approach by first leveraging the family of Ky Fan k-norms to derive a new class of algorithms, which we call Fanions. Subsequently, we create a second, hybrid family named F-Fanions, by constructing convex combinations of the Ky Fan norms with the Frobenius norm and taking dual of that composite norm. Working within the linear minimization oracle (lmo) framework~--- which is equivalent (up to a factor) to the trust region approach~--- we derive the explicit update formulas for both algorithm families. Later in \cref{sec:matrix-side} we explain how their updates can be approximated with the same $\cO(m n \min\{m,n\})$ complexity as Muon updates are computed via Newton-Schulz iterations.}{we focus on the two most common matrix norms akin to the spectral norm, namely the nuclear norm and the Frobenius norm. Working in the linear minimization oracle (lmo) framework, which is equivalent to a factor to the trust region approach and the steepest descent under norm constraint, we derive Neon, our algorithm based on the nuclear norm. In the section {\it Matrix side of the updates}, we explain how Neon updates can be computed asymptotically with the same $\cO(m n \min\{m, n\})$ number of FLOPs as Muon updates are computed via Newton-Schulz iterations.}
