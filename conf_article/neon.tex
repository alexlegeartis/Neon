...
Most bounds for Muon and other LMO-algorithms are given for the norm of the gradient \citep{kovalev2025understanding, pethick2025training, riabinin2025gluon,kovalev2025noneuclideansgdstructuredoptimization}. Accordingly, one is immediately interested to measure those norms on a real deep learning problem.

We compare Muon, F-Muon, S-Muon, NSGD, SignSGD, Neon, and F-Neon on CIFAR-airbench. NSGD and SignSGD are not heavily tuned. To preserve the algorithms, we do not normalize the weights of the network each iteration. The parameters are in the table.
...
\begin{table}[ht]
	...
	\begin{tabular}{|c|c|c|c|}
	  ...
	  \hline
	\end{tabular}
  \caption{Parameters for CIFAR-airbench. \texttt{sign\_lr\_mult}=0.003}
  \label{tbl:cifar_lrs}
  \end{table}
