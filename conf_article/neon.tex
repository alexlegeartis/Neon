...
\subsection{\replaced[id = IK, comment={Merged what previously had been section with two subsections into this one and improved the notation. Also removed the thing about trust region, it'll be referenced in succeeding sections.}]{Linear Minimization Oracle}{Problem Statement}}
...
\end{equation}
In practice, neural networks have multiple weight matrices $\mX_1, \mX_2, \ldots$, and the objective function depends on all of them. The framework we consider applies to each weight matrix independently, where the gradient $\mG_i = \nabla_{\mX_i} F$ for each matrix is computed via backpropagation, and the update rule is applied separately to each matrix.
...
\end{equation}
where $\mG$ is typically a gradient (possibly with momentum) of $F$ and $\cS \subset \Rmn$ is some set. The update rule used in Muon optimizer is uSCG \citep{pethick2025training}. In the most general case, which involves momentum and stochastic gradient $g(\mX^k, \xi^k)$, it can be written as
